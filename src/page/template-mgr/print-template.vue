	...
	<div class="market-mgr">
		...
		<toolbar class="custom-toolbar">
			...
			<span slot="right">
				...
				<el-button icon="el-icon-delete" size="small" style="width: 80px;" :disabled="marketForm==null||marketForm.isAble=='已启用'" v-show="!editModel" @click="doDelete">删除</el-button>
				<el-button icon="el-icon-setting" size="small" style="width: 100px;" :disabled="marketForm==null" v-show="!editModel" @click="doAbleOrDisable(marketForm.isAble==null?'启/停用':marketForm.isAble=='已停用'?'启用':'停用')">
					{{marketForm.isAble==null?'启/停用':marketForm.isAble=='已停用'?'启用':'停用'}}
				</el-button>
				<el-button icon="el-icon-setting" size="small" style="width: 100px;" :disabled="marketForm==null||marketForm.isAble=='已启用'" v-show="!editModel" @click="test">模版设计</el-button>
				<el-button icon="el-icon-setting" size="small" style="width: 100px;" :disabled="marketForm==null||marketForm.isAble=='已启用'" v-show="!editModel" @click="test">测试短信</el-button>
...
// 		ableOrDisableMarket
// 	} from '../../api/mac-mgr.js'
	import MarketForm from '../../components/template-mgr/print-template/form.vue'
	export default {
		components: {
			MarketForm
		},
		...
		methods: {
			...
			doDelete() {
				this.$confirm('您确认删除模版, 是否继续?', '系统提示', {
			...
			doSave() {
				if(this.marketForm.code==null||this.marketForm.code==''){
					this.$message({
						message: '请输入模版编码',
						type: 'warning'
					});
					return ;
				}else if(this.marketForm.name==null||this.marketForm.name==''){
					this.$message({
						message: '请输入模版名称...',
						type: 'warning'
					});
					return ;
				}if(this.marketForm.classList==null||this.marketForm.classList.length==0){
					this.$message({
					message: '请进行参数维护...',
					type: 'warning'
					});
